...
def CommandsList():
    ...
    os.startfile("D:\\Abhijeet\\VIV\\Commands List.txt")
...
def greet():
    conn = sqlite3.connect('VIV_DB.db')
    ...
    try:
        while(True):
            ...
            #asking for name
                ...
                SR.speak("My name is VIV and I'm here to serve you.")
            #How are you
            ...
            #calendar
                ...
                SR.updating_ST(calendar.calendar(2022))
            ...
            #whatsapp message
                ...
                whatsapp=WhatsApp(scrollable_text)
                whatsapp.send()
                del whatsapp
            #what is meant by
            ...
            #flipping coin
                ...
                #playsound.playsound('D:\\Abhijeet\\VIV\\quarter spin flac.mp3')
                SR.speak("It's " + cmove)
            ...
            #opening software applications
                ...
                codepath=r"C:\Users\Home\AppData\Local\Programs\Microsoft VS Code\Code.exe"
                ...
                os.startfile(r'D:\Program Files\WhatsApp.exe')
                break
...
if __name__=="__main__":
        ...
        root.geometry("{}x{}+{}+{}".format(920,500,int(root.winfo_screenwidth()/2 - 920/2),int(root.winfo_screenheight()/2 - 500/2)))
        
        root.resizable(0,0)
        root.title("VIV")
        root.iconbitmap(r"D:\\Abhijeet\\VIV\\VIV_icon.ico")
        root.configure(bg='#2c4557')
        scrollable_text=scrolledtext.ScrolledText(root,state='disabled',height=20,width=87,relief='sunken',bd=5,wrap=tk.WORD,bg='#add8e6',fg='#800000')
        scrollable_text.place(x=10,y=10)
        mic_img=Image.open(r"D:\\Abhijeet\\VIV\\Mic.png")
        ...
        Speak_label.place(x=350,y=440)
        """Setting up objects"""
        SR=SpeakRecog(scrollable_text)    #Speak and Recognition class instance
        Listen_Button=tk.Button(root,image=mic_img,borderwidth=0,activebackground='#2c4557',bg='#2c4557',command=Launching_thread)
        Listen_Button.place(x=420,y=420)
        myMenu=tk.Menu(root)
        m1=tk.Menu(myMenu,tearoff=0) #tearoff=0 meAnnex the submenu can't be teared of from the window
        m1.add_command(label='Commands List',command=CommandsList)
        myMenu.add_cascade(label="Help",menu=m1)
        stng_win=SettingWindow()
